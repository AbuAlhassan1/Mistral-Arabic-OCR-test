...
تم إضافة واجهة برمجة تطبيقات (API) احترافية باستخدام **FastAPI** مع المكونات الأساسية:

### ✨ ميزات API

- **📡 RESTful API**: واجهة برمجة تطبيقات كاملة مع توثيق تلقائي
- **🗄️ قاعدة بيانات PostgreSQL**: تخزين دائم للوثائق والوظائف
- **🔄 معالجة غير متزامنة**: دعم للمعالجة في الخلفية
- **📊 تتبع الحالة**: متابعة حالة كل عملية OCR
- **🔍 استعلامات متقدمة**: البحث والتصفية والترتيب
- **🐳 Docker Support**: جاهز للتشغيل باستخدام Docker

### 📚 التوثيق الكامل

راجع ملف [`API_DOCUMENTATION.md`](API_DOCUMENTATION.md) للحصول على التوثيق الكامل للـ API.
...
# نسخ ملف الإعدادات
cp env.example .env
# تعديل .env وإضافة MISTRAL_API_KEY
...
├── uploads/               # مجلد رفع الملفات (للـ API)
├── exports/               # مجلد الملفات المعالجة (للـ API)
